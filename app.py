...
app_ui = ui.page_fluid(
    ...
    ui.tags.head(
        ...
        (function() {
            ...
            /*
               Keep the three non-month rows at exactly the same
               rendered height.

               The month row is intentionally excluded because
               it already has its own fixed 27px height.

               For each corresponding row, all three tables are
               measured first with no synchronization height
               applied. The largest natural height is then used
               for all three tables. This prevents a cell in one
               section from forcing only that section to become
               taller.

               The height is applied to the row AND every cell,
               using the same fractional-pixel value.
            */
            ...
            function syncOneSegment(
                ...
                /*
                   Remove ONLY the heights managed by this
                   synchronizer. This gives every table its
                   natural intrinsic row height again before
                   measuring.
                */
                ...
                /*
                   Let the browser complete the natural table
                    ...
                    function() {
                        ...
                        ROW_SELECTORS.forEach(
                            function(selector) {
                                ...
                                /*
                                   Measure all three natural rows.
                                   Using the maximum guarantees that
                                   no section needs a larger row after
                                   synchronization.
                                */
                        ...
                        /*
                           The replenishment content is kept inside
                           the synchronized quantity row.
                        */

                        const rightQuantityRow =
                            tables[2].querySelector(
                                "tbody tr:first-child"
                            );
            ...
            /*
               Shiny redraws the scenario tables when the
               destination changes.
            */
            ...
            /*
               Re-run when the browser changes the dimensions of
